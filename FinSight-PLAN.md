...
- [x] **1.4** 编写预处理脚本：清洗（字段错位/乱码）+ 统一Alpaca格式
- [x] **1.5** 构建训练集（15.97万条训练 + 1.51万条评估）
...
### 2026-06-12
- [x] 完成预处理脚本 data/preprocess.py：
  - 券商广告整句清除（关键词匹配整句删除，枚举文案不可靠——变体太多）
  - FINFE情感标签归一（统一为"积极/消极/中性"，多标签/无标签样本剔除）
  - BAAI筛选：中文 + deita质量分>=7 + 首轮对话转Alpaca
  - 去重1,085条、超长15条、空字段4条；复检广告/乱码残留0条
- [x] 产出：finsight_train.jsonl 159,727条 / finsight_eval.jsonl 15,125条
  - agent分布: event 53.3k / qa 37.6k / summary 28.8k / topic 20.3k / sentiment 19.6k
- [ ] 下一步：LLaMA-Factory训练配置 + AutoDL租卡启动训练
